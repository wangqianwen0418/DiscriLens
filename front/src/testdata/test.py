...
def addItems(rule_file, sample_file):
    samples = pd.read_json(sample_file)
...
def modifyConf(rule_file):
    # rule_file = './academic_xgb_rules.json'
    rules = pd.read_json(rule_file)
    for idx, rule in rules.iterrows():
        sup_pnd = int(rule['sup_pnd'] - rule['sup_pd'])
        if sup_pnd==0:
            conf_pnd = 0
        elif len(rule['items'])- rule['sup_pd']/rule['conf_pd'] ==0:
            print(rule)
        else:
            conf_pnd = sup_pnd / (len(rule['items'])- rule['sup_pd']/rule['conf_pd'] )
        risk_dif = conf_pnd - rule['conf_pd']
        elift = conf_pnd/rule['conf_pd']

        rules.at[idx, 'sup_pnd'] = sup_pnd
        rules.at[idx, 'conf_pnd'] = conf_pnd
        rules.at[idx, 'risk_dif'] = risk_dif
